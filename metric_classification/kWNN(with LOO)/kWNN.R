...
looMatrix = looFuncOptKwnn(1, 150, 1, 
               0.05,1,0.05,
               iris30, kNN)

min_array = which(looMatrix == min(looMatrix), arr.ind = TRUE)

min_matrix = matrix( rep( '0', len=length(seq(1, 150, 1))*length(seq(0.05,1,0.05))), nrow = length(seq(1, 150, 1)))

for(index in c(1:(length(min_array)/2))){
  min_matrix[min_array[index,1], min_array[index,2]] = 'min'
}

rownames(min_matrix) = seq(1, 150, 1)
colnames(min_matrix) = seq(0.05,1,0.05)
# print(min_matrix)

opt_k = min_array[1,1]
opt_q = min_array[1,2]/20
